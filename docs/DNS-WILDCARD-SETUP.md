# Setup do DNS Wildcard (*.axcend.com)

Pra cada cliente do Axcend ter seu próprio subdomínio (`clienteX.axcend.com`),
...
5. Cola exatamente: `*.axcend.com`
6. Salva
7. Railway vai te dar uma instrução tipo:
   ```
   CNAME *.axcend.com → kbb1d4hp.up.railway.app
   ```
...
```bash
# No terminal:
dig +short qualquer-coisa.axcend.com
# Deve retornar o IP do Railway

# Ou via web:
# https://dnschecker.org/#CNAME/qualquer-coisa.axcend.com
```
...
Se você quiser que o seu painel ADMIN do Axcend continue acessível em `app.centralaxcend.com` (não conflitando com o `*.axcend.com`), garante que está no `HOSTS_INTERNO`:
...
const HOSTS_INTERNO = new Set([
  ...
  'www.axcend.com',     // ← www também
  'app.axcend.com',     // ← se quiser que app.axcend.com seja master
  'localhost:3001',
  'localhost:3000',
  '127.0.0.1:3001'
]);
...
4. Após signup, é redirecionado pra `https://meuteste.axcend.com`
...
3. Cliente é redirecionado pra `https://meuteste.axcend.com`
4. Login automático funciona
5. Painel isolado mostra só dados do cliente
6. Outros clientes (`outroteste.axcend.com`) não veem nada desse
